...
void init_bootstrap_buffers() {
    ...
    for (i = 0; i < NB_BOOTSTRAP_BUFFERS; i++) {
        ...
        assert( ((unsigned long long)buf_addr) % MEM_ALIGN_CONSTRAINT == 0 );
...
int is_bootstrap_buffer(void *p) {
    int res = 0;
    debug_printf("enter - p = %p\n", p);
    if (((char*)p >= (char*)(bootstrap_buffers[0].buf)) && ((char*)p < ((char*)(bootstrap_buffers[NB_BOOTSTRAP_BUFFERS-1].buf)+BOOTSTRAP_BUFFER_SIZE-1))) {
        res = 1;
    }
...
void *realloc(void *ptr, size_t size){
    ...
    if (find_pool_from_address(ptr) == -1) {
        /* 
